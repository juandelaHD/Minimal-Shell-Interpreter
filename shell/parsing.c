...
{
	// Your code here
	if (arg[0] == '$') {  // comprobamos que arg comience por $
		// obtenemos el valor de la variable de entorno (arg + 1 para saltar el $)
		if (arg[1] == '?') {
			...
			if (env == NULL) {
				// si no existe la variable de entorno dejamos a arg como una cadena vacia
				arg = (char *) realloc(arg, 1);
				arg[0] = END_STRING;
			} else {
				// si exite la variable de entorno entonces la
				// realocamos en arg con su tamaño correspondiente
...
parse_line(char *buf)
{
	...
	// if i have multiple pipes, i will parse the right
	// part of the command recursively
	if (block_contains(right, '|') >= 0) {
		r = parse_line(right);  // returnea pipe_cmd
	} else {
		r = parse_cmd(right);
	}
	l = parse_cmd(buf);
